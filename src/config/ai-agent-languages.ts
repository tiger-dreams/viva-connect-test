...
export type AgentLanguage = 'ko' | 'en' | 'ja' | 'th' | 'zh-TW';

/** Room ID → agent language mapping */
export const ROOM_LANGUAGE_MAP: Record<string, AgentLanguage> = {
  korea: 'ko',
  japan: 'ja',
  taiwan: 'zh-TW',
  thailand: 'th',
...
/**
 * Determine agent language from roomId.
 * Falls back to UI language ('ko'/'en') for custom rooms.
 */
export function getAgentLanguage(roomId: string, uiLanguage: string): AgentLanguage {
  return ROOM_LANGUAGE_MAP[roomId] ?? (uiLanguage === 'ko' ? 'ko' : 'en');
}
