...
const BetaSetupPage = () => {
  ...
  const isBeta = true; // Beta 전용 Setup 페이지
  ...
  useEffect(() => {
    ...
    if (roomParam) {
      ...
      console.log('[BetaSetupPage] Deep link room parameter detected:', roomValue);
      ...
      if (presetRooms.includes(roomValue)) {
        ...
        console.log('[BetaSetupPage] Auto-selected preset room:', roomValue);
      } else {
        // Custom room
        setSelectedRoomType('custom');
        setCustomRoomId(roomParam); // 원본 대소문자 유지
        setPlanetKitConfig(prev => ({ ...prev, roomId: roomParam, accessToken: '' }));
        console.log('[BetaSetupPage] Auto-selected custom room:', roomParam);
      }
    }
  ...
  useEffect(() => {
    ...
    console.log('[BetaSetupPage] Auto-token useEffect triggered', { status, mode: modeParam, sid: sidParam });
    ...
    if (roomParam && isLoggedIn && profile && planetKitConfig.roomId && !planetKitConfig.accessToken && !autoTokenGeneratedRef.current) {
      ...
      if (planetKitConfig.serviceId && planetKitConfig.apiKey && planetKitConfig.userId) {
        ...
        console.log('[BetaSetupPage] Auto-generating token for deep link entry...', { isAgentCall });
        ...
        generatePlanetKitToken(
          ...
          console.log('[BetaSetupPage] Token auto-generated successfully');
          ...
          setTimeout(() => {
            if (isAgentCall && sidParam) {
              console.log('[BetaSetupPage] Auto-navigating to beta agent call meeting...', { sid: sidParam, cc_param: ccParamValue ? 'present' : 'missing' });
              setDebugInfo(prev => prev ? { ...prev, status: '🚀 Navigating to beta agent call...' } : null);
          ...
          console.error('[BetaSetupPage] Auto token generation failed:', error);
